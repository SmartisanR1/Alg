  ...
  <PageLayout title="证书管理" subtitle="CSR 生成 · 证书签发 · 自签名证书 · 国际/国密支持"
    ...
    <div v-show="activeTab === 'parse'" class="flex flex-col lg:flex-row gap-4 animate-fade-in h-full">
      <div class="w-full lg:w-1/3 space-y-3 shrink-0">
        <Card title="PEM 证书解析" class="space-y-3">
          <div class="flex gap-2 mb-2">
            <Button variant="secondary" class="flex-1 justify-center text-[11px] py-1.5" @click="uploadCertFile">
          ...
          <Button variant="primary" block @click="parseCert" class="py-2">解析证书</Button>
        </Card>
      </div>
      <div class="flex-1 min-w-0">
        <Card title="解析结果" class="h-full flex flex-col cert-result">
          <ResultArea
            :modelValue="certResult.data"
    ...
    <div v-show="activeTab === 'csr'" class="sym-workbench animate-fade-in">
      <div class="sym-side">
        <Card title="CSR 请求信息" class="space-y-5">
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              <Input v-model="csr.o" label="组织 (O)" placeholder="Organization" />
            </div>
            <div>
              <Input v-model="csr.c" label="国家 (C)" placeholder="CN" />
            </div>
            <div>
          ...
          <div class="pt-1">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <Dropdown
                v-model="csr.type"
                :options="[
                  { value: 'both', label: '通用 (签名+加密)' },
                  { value: 'sign', label: '仅签名' },
                  { value: 'encrypt', label: '仅加密' }
                ]"
                :disabled="csr.algo !== 'SM2'"
              />
            </div>
          </div>
      ...
      <div class="sym-main !min-w-0">
        <Card title="CSR 结果" class="flex flex-col cert-result">
          <ResultArea
            :modelValue="csrResult.data"
            :error="csrResult.error"
            :success="csrResult.success"
            label="生成的 CSR (PEM)"
            copyable
          />
    ...
    <!-- CSR Signing -->
    <div v-show="activeTab === 'sign'" class="sym-workbench animate-fade-in">
      <div class="sym-side">
        <div class="card space-y-4">
          <p class="card-title">上传 CSR 签发</p>
            ...
            <div v-if="signShowAdvanced" class="space-y-4 p-3 rounded-lg border transition-colors"
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="flex items-center gap-2">
                  <label class="input-label !mb-0">基本约束:</label>
                ...
                <div v-if="signReq.isCA">
                  <label class="input-label">路径长度限制</label>
                  <input v-model.number="signReq.pathLen" type="number" class="input !h-7 !py-0" placeholder="-1 (无限制)" />
                </div>
              </div>
              <div>
                <label class="input-label">使用者备用名称 (SAN)</label>
                <textarea v-model="signReq.sanRaw" class="input !h-16 text-[10px]" placeholder="每行一个 DNS:www.domain.com 或 IP:1.1.1.1"></textarea>
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="input-label">CRL 分发点 (CDP)</label>
                  <textarea v-model="signReq.crlRaw" class="input !h-16 text-[10px]" placeholder="http://crl.example.com/ca.crl"></textarea>
                </div>
                <div>
                  <label class="input-label">OCSP 服务地址</label>
                  <textarea v-model="signReq.ocspRaw" class="input !h-16 text-[10px]" placeholder="http://ocsp.example.com"></textarea>
                </div>
              </div>

              <div>
                <label class="input-label">证书策略 (Policy OIDs)</label>
                <textarea v-model="signReq.policyRaw" class="input !h-12 text-[10px]" placeholder="例如 2.5.29.32.0 (每行一个 OID)"></textarea>
              </div>

              <div>
                <label class="input-label">增强密钥用途</label>
          ...
          <div class="grid grid-cols-3 gap-3">
            ...
            <div>
              <label class="input-label">证书用途</label>
              <Dropdown
                v-model="signReq.type"
                :options="[
                  { value: 'both', label: '通用 (签名+加密)' },
                  { value: 'sign', label: '仅签名' },
                  { value: 'encrypt', label: '仅加密' }
      ...
      <div class="sym-main !min-w-0">
        <div class="card flex flex-col cert-result">
          ...
          <div v-if="signResult.success" class="mt-4 flex gap-2">
            <button @click="downloadFile(signResult.data, 'certificate.cer')" class="btn-primary flex-1 justify-center text-xs">
    ...
    <div v-show="activeTab === 'self'" class="sym-workbench animate-fade-in">
      <div class="sym-side">
        <div class="card space-y-4">
          <div class="flex justify-between items-center">
            <p class="card-title">直接签发 (由内置 CA 签名)</p>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="input-label">通用名称 (CN)</label>
              <input v-model="selfReq.cn" class="input" placeholder="www.example.com" />
            </div>
            <div>
              <label class="input-label">组织 (O)</label>
              <input v-model="selfReq.o" class="input" placeholder="CryptoKit User" />
            </div>
            <div>
              <label class="input-label">有效期 (天)</label>
              <input v-model.number="selfReq.days" type="number" class="input" />
            </div>
            <div>
              <label class="input-label">签名算法</label>
              <Dropdown
                v-model="selfReq.algo"
                :options="[
                  { value: 'RSA', label: 'RSA (2048)' },
                  { value: 'ECC', label: 'ECC (P-256)' },
                  { value: 'SM2', label: '国密 SM2' }
                ]"
              />
            </div>
            ...
            <div v-if="selfShowAdvanced" class="space-y-4 p-3 rounded-lg border transition-colors"
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div v-if="selfReq.isCA">
                  <label class="input-label">路径长度限制</label>
                  <input v-model.number="selfReq.pathLen" type="number" class="input !h-7 !py-0" placeholder="-1 (无限制)" />
                </div>
              </div>

              <div>
                <label class="input-label">使用者备用名称 (SAN)</label>
                <textarea v-model="selfReq.sanRaw" class="input !h-16 text-[10px]" placeholder="DNS:example.com&#10;IP:127.0.0.1"></textarea>
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="input-label">CRL 分发点 (CDP)</label>
                  <textarea v-model="selfReq.crlRaw" class="input !h-16 text-[10px]" placeholder="http://crl.example.com/ca.crl"></textarea>
                </div>
                <div>
                  <label class="input-label">OCSP 服务地址</label>
                  <textarea v-model="selfReq.ocspRaw" class="input !h-16 text-[10px]" placeholder="http://ocsp.example.com"></textarea>
                </div>
              </div>

              <div>
                <label class="input-label">证书策略 (Policy OIDs)</label>
                <textarea v-model="selfReq.policyRaw" class="input !h-12 text-[10px]" placeholder="例如 2.5.29.32.0 (每行一个 OID)"></textarea>
              </div>

              <div>
                <label class="input-label">密钥用途 (Key Usage)</label>
              ...
              <div>
                <label class="input-label">增强用途 (Ext Key Usage)</label>
                <div class="grid grid-cols-2 gap-x-2 gap-y-1">
                  <label v-for="eku in extKeyUsageOptions" :key="eku.value" class="flex items-center gap-2 text-[10px] hover:text-violet-400 cursor-pointer"
                         :class="isDark ? 'text-dark-muted' : 'text-gray-500'">
          ...
          <button @click="genSelfSigned" class="btn-success w-full justify-center mt-2">立即签发并生成结果</button>
        </div>
      </div>
      <div class="sym-main !min-w-0">
        <!-- Certificate Result -->
        <div class="card">
          <div class="flex justify-between items-center mb-1">
            <label class="input-label !mb-0 text-emerald-400">生成的证书 (PEM)</label>
        ...
        <div class="card">
          <div class="flex justify-between items-center mb-1">
            <label class="input-label !mb-0 text-violet-400">生成的 CSR (PEM)</label>
            <div class="flex gap-1">
        ...
        <div class="card">
          <div class="flex justify-between items-center mb-1">
            <label class="input-label !mb-0 text-orange-300">配套私钥 (PEM)</label>
            <div class="flex gap-1">
    ...
    <div v-show="activeTab === 'dual'" class="sym-workbench animate-fade-in">
      <div class="sym-side overflow-y-auto pr-1 custom-scrollbar">
        <div class="card space-y-4">
          <div class="flex justify-between items-center">
            <p class="card-title">国密双证书签发 (签名+加密)</p>
            <button @click="downloadRootCert('SM2')" class="text-[10px] px-2 py-1 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 transition-all flex items-center gap-1">
              <DownloadIcon class="w-3 h-3" /> SM2 根证书
            </button>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="input-label">通用名称 (CN)</label>
              <input v-model="dualReq.cn" class="input" placeholder="www.example.com" />
            </div>
            <div>
              <label class="input-label">组织 (O)</label>
              <input v-model="dualReq.o" class="input" placeholder="CryptoKit User" />
            </div>
            <div>
              <label class="input-label">有效期 (天)</label>
              <input v-model.number="dualReq.days" type="number" class="input" />
            </div>
            <div>
              <label class="input-label">签名算法</label>
              <Dropdown
                v-model="dualReq.algo"
                :options="[
                  { value: 'SM2', label: '国密 SM2 (标准要求)' }
                ]"
                disabled
              />
            </div>
          ...
          <transition name="fade">
            <div v-if="dualShowAdvanced" class="space-y-4 p-3 rounded-lg border transition-colors"
              ...
              <div>
                <label class="input-label">使用者备用名称 (SAN)</label>
                <textarea v-model="dualReq.sanRaw" class="input !h-16 text-[10px]" placeholder="DNS:example.com&#10;IP:127.0.0.1"></textarea>
              </div>

              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="input-label">CRL 分发点 (CDP)</label>
                  <textarea v-model="dualReq.crlRaw" class="input !h-16 text-[10px]" placeholder="http://crl.example.com/ca.crl"></textarea>
                </div>
                <div>
                  <label class="input-label">OCSP 服务地址</label>
                  <textarea v-model="dualReq.ocspRaw" class="input !h-16 text-[10px]" placeholder="http://ocsp.example.com"></textarea>
                </div>
              </div>

              <div>
                <label class="input-label">证书策略 (Policy OIDs)</label>
                <textarea v-model="dualReq.policyRaw" class="input !h-12 text-[10px]" placeholder="例如 2.5.29.32.0 (每行一个 OID)"></textarea>
              </div>
            </div>
          </transition>

          <button @click="genDualCerts" class="btn-success w-full justify-center mt-2 shadow-lg shadow-violet-500/10">立即签发双证书及信封</button>
        </div>
        
        <!-- Inline Principle for Dual Cert -->
        <div class="card bg-gradient-to-br from-violet-500/5 to-transparent border-violet-500/10 shrink-0">
          <p class="card-title text-violet-400">双证书体系说明 (GM/T 0010)</p>
        ...
        <div v-if="dualResult.success" class="flex-1 min-h-0 flex flex-col space-y-3 animate-fade-in">
          <div class="grid grid-cols-2 gap-3 shrink-0">
            <div class="card flex flex-col p-2.5">
              ...
              <textarea readonly class="result-area !min-h-[100px] !text-[9px] font-mono resize-none bg-transparent border-none outline-none overflow-y-auto" v-model="dualResult.signCert"></textarea>
            </div>
            <div class="card flex flex-col p-2.5">
              <div class="flex justify-between items-center mb-1.5">
              ...
              <textarea readonly class="result-area !min-h-[100px] !text-[9px] font-mono resize-none bg-transparent border-none outline-none overflow-y-auto" v-model="dualResult.encryptCert"></textarea>
            </div>
          </div>
          
          <div class="card flex-1 min-h-0 flex flex-col p-3">
            <div class="flex justify-between items-center mb-2 shrink-0">
              <span class="text-[11px] font-bold text-orange-300 flex items-center gap-1.5">
                <ShieldCheckIcon class="w-3.5 h-3.5" /> 加密私钥信封
              </span>
              <button @click="downloadFile(dualResult.enwrappedKey, 'enc_key.env')" class="ck-copy-btn !bg-amber-500/10 !text-orange-300">下载 .env</button>
            </div>
          ...
          <div class="card shrink-0 flex flex-col p-3">
            <div class="flex justify-between items-center mb-2 shrink-0">
            ...
            <textarea readonly class="result-area !min-h-[60px] !max-h-[100px] !text-[10px] font-mono resize-none bg-transparent border-none outline-none overflow-y-auto" v-model="dualResult.signKey"></textarea>
          </div>
        </div>
        <div v-else class="card h-full flex flex-col items-center justify-center text-dark-muted space-y-4 border-dashed opacity-60">
