...
/* 导航标签 - 精致 */
.tool-pill-new {
  @apply flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium transition-all cursor-pointer whitespace-nowrap;
  border: 1px solid transparent;
}

.app-container.dark .tool-pill-new {
   @apply text-dark-muted hover:text-white hover:bg-white/5;
 }
...
/* 标题栏按钮 - 精致 */
.titlebar-icon-btn {
  @apply w-6 h-6 rounded-md inline-flex items-center justify-center transition-colors border;
}
...
.toolbar-select-label {
  @apply absolute left-2.5 top-1/2 -translate-y-1/2 text-xs font-semibold pointer-events-none opacity-70;
}

.toolbar-select {
  @apply h-7 min-w-[120px] rounded-md border pl-9 pr-7 text-xs font-semibold outline-none appearance-none;
...
.tool-pill {
  @apply shrink-0 inline-flex items-center gap-1.5 px-2 py-1 h-7 rounded-md border text-xs font-medium transition-all duration-200;
}
...
.page-stage-light {
  @apply bg-white/95 border-light-border shadow-sm shadow-slate-200/70;
}
